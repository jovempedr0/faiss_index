...
| `auto_index_thresholds` | `(flat_limit, ivf_flat_limit)` used when `index_type="auto"`. Defaults to `(100_000, 250_000)`. |
...
Where the two limits sit is a measured trade, not a round number. Exact search over
1024-dimensional vectors costs 1.90 ms/query at 50k, 3.80 ms at 100k, 9.61 ms at 250k
and 18.95 ms at 500k, and `flat` and `ivf_flat` occupy the same memory (both keep the
full vectors) — so below 100k the approximation buys about 3 ms and pays for it in
recall, which is the wrong way round for a default. `ivf_sq8` takes over at 250k, where
storing a quarter of the bytes (489 MiB against 1957 at 500k) is worth its ~1 point of
recall, which comes from the quantization and not from `nprobe`.
...
topics than the 23 the fixture corpus has. The two harder columns land near the 95.3%
